...
def send_curl_every_second(sensor_id, sensor_value, interval):
    while True:
        ...
        subprocess.run([
            ...
            '-d',f'{{"sensor_id": {sensor_id}, "date":"{datetime.now().strftime("%Y-%m-%d")}", "time":"{datetime.now().strftime("%H:%M:%S")}", "sensor_value": {sensor_value}}}', 
            'http://192.168.70.89:9000/data-endpoint'
        ])
        time.sleep(interval)

# 함수 실행
send_curl_every_second(sensor_id='1', sensor_value=100, interval=3)
